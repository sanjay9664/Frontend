...
const Dashboard = () => {
  ...
  const mockZones = { 
    'Zomato': ['Zomato_NCR', 'Zomato_Delhi']
  };
  const mockAreas = { 
    'Zomato_NCR': ['Ghaziabad', 'Noida'], 
    'Zomato_Delhi': ['East Delhi', 'South Delhi'] 
  };
  const mockLocations = { 
    'Ghaziabad': ['Ghazibad_1', 'Ghazibad_2'], 
    'Noida': ['Noida_1'], 
    'East Delhi': ['East Delhi_1'], 
    'South Delhi': ['South Delhi_1']
  };
  const mockUnits = { 
    'Ghazibad_1': ['STORE-1', 'STORE-3'], 
    'Ghazibad_2': ['STORE-4'], 
    'Noida_1': ['STORE-5'], 
    'East Delhi_1': ['Store-2'], 
    'South Delhi_1': ['STORE-6'] 
  };

  const orgOptions = ['Zomato'];
  ...
  useEffect(() => {
    ...
    let org = localStorage.getItem('scada_selectedOrg');
    let zone = localStorage.getItem('scada_selectedZone');
    let area = localStorage.getItem('scada_selectedArea');
    let loc = localStorage.getItem('scada_selectedLocation');
    let unit = localStorage.getItem('scada_selectedUnit');

    if (selectedRole === 'Super Admin') {
      if (!org || !orgOptions.includes(org)) org = 'Zomato';
    } else {
      org = 'Zomato';
    }

    if (['Zone Manager', 'Area Manager', 'Location Manager', 'Unit Head', 'Operator'].includes(selectedRole)) {
      zone = 'Zomato_NCR';
    } else if (!zone || !mockZones[org]?.includes(zone)) {
      zone = '';
    }

    if (['Area Manager', 'Location Manager', 'Unit Head', 'Operator'].includes(selectedRole)) {
      area = 'Ghaziabad';
    } else if (!area || !mockAreas[zone]?.includes(area)) {
      area = '';
    }

    if (['Location Manager', 'Unit Head', 'Operator'].includes(selectedRole)) {
      loc = 'Ghazibad_1';
    } else if (!loc || !mockLocations[area]?.includes(loc)) {
      loc = '';
    }

    if (['Unit Head', 'Operator'].includes(selectedRole)) {
      unit = 'STORE-1';
    } else if (!unit || !mockUnits[loc]?.includes(unit)) {
      unit = '';
    }
  ...
  const isAggregateView = !selectedUnit;
  ...
  return (
    <div className="dashboard-wrapper p-4">
      ...
      <div className="scope-filter-panel rounded-4 p-3 mb-4 d-flex flex-wrap gap-3 align-items-center position-relative shadow-lg">
        <span className="text-info-scada fs-11 fw-black tracking-widest text-uppercase d-flex align-items-center position-relative z-1">
          <Network size={14} className="me-2"/> Scope Filters:
        </span>

        <Dropdown>
          <Dropdown.Toggle variant="dark" className="scada-dropdown-toggle py-1 px-3 fs-11" disabled={selectedRole !== 'Super Admin'}>
          ...
          {selectedRole === 'Super Admin' && (
            <Dropdown.Menu variant="dark" className="scada-dropdown-menu">
              {orgOptions.map(org => (
                <Dropdown.Item key={org} onClick={() => handleOrgSelect(org)} className="scada-dropdown-item fs-11">
                  {org}
                </Dropdown.Item>
              ))}
            </Dropdown.Menu>
        ...
        <Dropdown>
          <Dropdown.Toggle variant="dark" className="scada-dropdown-toggle py-1 px-3 fs-11" disabled={!selectedOrg || !['Super Admin', 'Organization Admin'].includes(selectedRole)}>
          ...
          {['Super Admin', 'Organization Admin'].includes(selectedRole) && (
            <Dropdown.Menu variant="dark" className="scada-dropdown-menu">
              {zoneOptions.map(zone => (
                <Dropdown.Item key={zone} onClick={() => handleZoneSelect(zone)} className="scada-dropdown-item fs-11">
                  {zone}
                </Dropdown.Item>
              ))}
            </Dropdown.Menu>
        ...
        <Dropdown>
          <Dropdown.Toggle variant="dark" className="scada-dropdown-toggle py-1 px-3 fs-11" disabled={!selectedZone || !['Super Admin', 'Organization Admin', 'Zone Manager'].includes(selectedRole)}>
          ...
          {['Super Admin', 'Organization Admin', 'Zone Manager'].includes(selectedRole) && (
            <Dropdown.Menu variant="dark" className="scada-dropdown-menu">
              {areaOptions.map(area => (
                <Dropdown.Item key={area} onClick={() => handleAreaSelect(area)} className="scada-dropdown-item fs-11">
                  {area}
                </Dropdown.Item>
              ))}
            </Dropdown.Menu>
        ...
        <Dropdown>
          <Dropdown.Toggle variant="dark" className="scada-dropdown-toggle py-1 px-3 fs-11" disabled={!selectedArea || !['Super Admin', 'Organization Admin', 'Zone Manager', 'Area Manager'].includes(selectedRole)}>
          ...
          {['Super Admin', 'Organization Admin', 'Zone Manager', 'Area Manager'].includes(selectedRole) && (
            <Dropdown.Menu variant="dark" className="scada-dropdown-menu">
              {locationOptions.map(loc => (
                <Dropdown.Item key={loc} onClick={() => handleLocationSelect(loc)} className="scada-dropdown-item fs-11">
                  {loc}
                </Dropdown.Item>
              ))}
            </Dropdown.Menu>
        ...
        <Dropdown>
          <Dropdown.Toggle variant="dark" className="scada-dropdown-toggle py-1 px-3 fs-11" disabled={!selectedLocation || !['Super Admin', 'Organization Admin', 'Zone Manager', 'Area Manager', 'Location Manager'].includes(selectedRole)}>
      ...
      {isAggregateView ? (
        <>
          {/* AGGREGATE VIEW */}
          <div className="mb-4">
            <h5 className="text-white fw-black tracking-widest uppercase mb-1">
              <Globe size={18} className="me-2 text-primary" /> Aggregate Regional Overview
            </h5>
            <p className="text-muted fs-12 uppercase fw-bold">Monitoring all sites under: <span className="text-info">{selectedLocation || selectedArea || selectedZone || selectedOrg || 'All Regions'}</span></p>
          </div>
          ...
          <Row className="g-4 mb-5">
            <Col xl={3} md={6}>
              <StatusCard
                ...
                value={dynamicMetrics.pump * 4 + 12}
                unit="Online"
            ...
            <Col xl={3} md={6}>
              <StatusCard
                ...
                value={dynamicMetrics.avgLoad * 8}
                unit="KW"
                icon={<Zap size={20} />}
                color="warning"
                trend="down"
                ...
                <div className="px-4 py-3 border-bottom border-white border-opacity-5 d-flex justify-content-between align-items-center bg-black bg-opacity-20">
                  <div>
                    <h6 className="mb-0 text-info-scada fw-black tracking-widest uppercase fs-12">
                      <Activity size={16} className="me-2" /> Regional Load Analysis
                    </h6>
                  </div>
                </div>
                <Card.Body className="p-4">
                  <div style={{ width: '100%', height: 300 }}>
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={dynamicChartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                        <defs>
                          <linearGradient id="colorPowerAgg" x1="0" y1="0" x2="0" y2="1">
                        ...
                        <Area type="monotone" dataKey="power" stroke="#8b5cf6" fillOpacity={1} fill="url(#colorPowerAgg)" strokeWidth={2} />
                      </AreaChart>
                    </ResponsiveContainer>
                  </div>
                </Card.Body>
        ...
        <>
          ...
          <div className="mb-4">
            <h5 className="text-white fw-black tracking-widest uppercase mb-1">
              <Cpu size={18} className="me-2 text-warning" /> Unit Telemetry Dashboard
            </h5>
            <p className="text-muted fs-12 uppercase fw-bold">Live Data for: <span className="text-warning">{selectedUnit}</span></p>
          </div>

          <Row className="g-4 mb-5">
            <Col xl={3} md={6}>
              <StatusCard title="PUMP STATUS" value={dynamicMetrics.pump} unit="Active" icon={<Activity size={20} />} color="success" trend="up" trendValue="+2" path="/motors/overview" />
            </Col>
            <Col xl={3} md={6}>
              <StatusCard title="TANKS STATUS" value={dynamicMetrics.tanks} unit="Low Level" icon={<Database size={20} />} color="danger" trend="down" trendValue="-1" path="/water-management/tanks" />
            </Col>
            <Col xl={3} md={6}>
              <StatusCard title="VALVE STATUS" value={dynamicMetrics.valve} unit="Open" icon={<Droplets size={20} />} color="info" trend="up" trendValue="98%" path="/water-management/valves" />
            </Col>
            <Col xl={3} md={6}>
              <StatusCard title="Critical Alerts" value={dynamicMetrics.alerts} unit="Unresolved" icon={<ShieldAlert size={20} />} color="danger" trend="down" trendValue="1 ACK" path="/alarm-system" />
            </Col>
          </Row>

          <Row className="g-4 mb-5">
            <Col xl={8}>
              <Card className="bg-panel border-0 shadow-lg rounded-4 overflow-hidden mb-4 h-100">
                <div className="px-4 py-3 border-bottom border-white border-opacity-5 d-flex justify-content-between align-items-center bg-black bg-opacity-20">
                  <div>
                    <h6 className="mb-0 text-info-scada fw-black tracking-widest uppercase fs-12">
                  ...
                  {[
                    ...
                    return (
                      <div 
                        ...
                        style={{ transition: 'all 0.2s ease', cursor: aqiAllowed ? 'pointer' : 'not-allowed' }}
      ...
      <style dangerouslySetInnerHTML={{
        ...
        .scope-filter-panel {
            ...
            background-image: linear-gradient(90deg, rgba(14, 165, 233, 0.05) 0%, rgba(2, 6, 23, 0) 100%);
        ...
        .scada-dropdown-toggle:disabled {
            opacity: 0.6;
            cursor: not-allowed;
            pointer-events: none;
        }
